...
class FlightComputerNode:
    ...
    def start(self):
        print(f"[INIT] Iniciando Computador de Voo (Node ID: {self.node_id})")
        ...
        try:
            ...
            print("[SHUTDOWN] Desligando sistema...")
    ...
    def heartbeat_loop(self):
        ...
        while True:
            if self.is_leader:
                ...
                print(f"[LÍDER] Ping enviado para health e keep-alive (Node {self.node_id})")
            time.sleep(3)
    ...
    def telemetry_loop(self):
        ...
        while True:
            if self.is_leader:
                ...
                print(f"[PRODUÇÃO] Guiamento publicado: {telemetry}")
            
            time.sleep(1) # Frequência de atualização da navegação

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Node do Computador de Voo')
    parser.add_argument('--id', type=int, required=True, help='ID do Nó (2 para Primário, 1 para Secundário)')
    args = parser.parse_args()

    node = FlightComputerNode(node_id=args.id)
    node.start()