"""
=================================================================
  Detector de Falhas — Heartbeat Monitor
  Integrante: Gabriela
=================================================================
  Escuta heartbeats dos módulos no tópico Kafka
  avionica.module.health e monitora o tempo entre sinais.
  Se um módulo ficar >15s sem heartbeat:
    - Marca como DOWN na tabela module_status (PostgreSQL)
    - Gera alerta CRITICAL no barramento de eventos
=================================================================
"""
...
# ── Mapeamento de nomes ──────────────────────────────────────
# Mapeia nomes recebidos via heartbeat -> nome no banco module_status
MODULE_NAME_MAP = {
...
def consumer_loop():
    ...
    for msg in consumer:
        ...
        # Aceitar vários formatos de payload
        ...
        # Se for node_id numérico (dos computadores de voo), traduzir
        if modulo_raw == "1":
        ...
        with lock:
            ...
            # Se o módulo estava DOWN, marcar como UP (recuperação)
        ...
        with lock:
            for modulo, last_ts in list(last_heartbeat.items()):
                ...
                if elapsed > TIMEOUT_SECONDS and current_status != "DOWN":
                    ...
                    # 2. Inserir alerta na tabela alertas
...
def main():
    print("=" * 60, flush=True)
    print("  💓  Detector de Falhas — Heartbeat Monitor (Gabriela)", flush=True)
    print("=" * 60, flush=True)
    print(f"  Timeout: {TIMEOUT_SECONDS}s | Intervalo de check: {CHECK_INTERVAL}s", flush=True)
    print(f"  Kafka: {KAFKA_BROKER} | DB: {DB_HOST}:{DB_PORT}/{DB_NAME}", flush=True)
    print("=" * 60, flush=True)
    ...
    try:
        ...
        for row in cur.fetchall():
            ...
            # Colocar um timestamp inicial para não disparar falso positivo
